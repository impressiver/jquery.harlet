...
(function($) {
    function Harlet(placeholder, inputData, options, plugins) {
            ...
            for(var i = 0, pL = data.pages.length; i < pL; ++i) {
                ...
                for(var j = 0, sL = page.slices.length; j < sL; ++j) {
                    ...
                    for(var k = 0, eL = slice.entries.length; k < eL; ++k) {
                        ...
                        var resolving = ((entry.timings.dns < 0) ? 0 : entry.timings.dns);
                        var connecting = resolving + ((entry.timings.connect < 0) ? 0 : entry.timings.connect);
                        var blocking = connecting + ((entry.timings.blocked < 0) ? 0 : entry.timings.blocked);
                        var sending = blocking + ((entry.timings.send < 0) ? 0 : entry.timings.send);
                        var waiting = sending + ((entry.timings.wait < 0) ? 0 : entry.timings.wait);
                        var receiving = waiting + ((entry.timings.receive < 0) ? 0 : entry.timings.receive);
        ...
        function showTooltip (x, y, entry) {
            ...
            $('<div class="harlet-tooltip" class="tooltip"></div>').append(content).css({
                position: 'absolute',
                top: y + 5,
                left: x + 5
